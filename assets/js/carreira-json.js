...
const renderCareerTimeline = (careerSteps) => {
	...
	careerSteps.forEach((step, index) => {
		...
		description.appendChild(createElement("h4", "resume-timeline-item-desc-heading font-weight-bold", "Competências comportamentais para essa etapa"));
...
const parseCareerData = (jsonText) => {
	try {
		...
		try {
			console.warn("JSON com vírgula sobrando detectado. Aplicando correção automática.");
...
fetch(DATA_URL)
	...
	.catch((error) => {
		if (window.CAREER_DATA) {
			renderPage(window.CAREER_DATA);
			console.info("Usando dados locais porque o JSON não foi carregado por fetch.");
			return;
		}
